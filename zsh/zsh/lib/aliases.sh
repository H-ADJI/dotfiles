alias ls="eza --color=always --git "
alias adb_root="adb shell setprop persist.sys.root_access 3 && adb root"
alias list_android_devices="gmtool admin list"
alias launch_android_device="/opt/genymotion/player  --vm-name 'Samsung Galaxy S23'"
alias gclean="git clean -f"
alias lc="leetcode"
alias nala_update="sudo nala update && sudo nala upgrade -y && sudo nala autoremove -y"
alias nala_install="sudo nala install"
alias e='$EDITOR'
alias nv="nvim"
alias nvconfig="cd ~/.config/nvim && nv"
alias nvim_shada_clear="rm ~/.local/state/nvim/shada/main.shada"
alias py="python3.10"
alias venv_create="py -m venv .venv"
alias venv_activate="source ./.venv/bin/activate"
alias tmuxconf="nv ~/.tmux.conf"
alias tn="tmux new -s"
alias install_tpm_plugins="~/.tmux/plugins/tpm/bin/install_plugins"
alias t="tmuxp load -y"
alias now="date +'%Y-%m-%d %H:%M:%S'"
alias csv_to_json="jq -Rsn '(input | split(\"\n\") | .[1:] | map(select(length > 0) | split(\",\") | {url: .[0], path: .[1], params: null}))' > paths.json"
alias c="clear"
alias ..="cd .."
alias mkdir="mkdir -pv"
alias grep="grep --color=auto"
alias copy="wl-copy"
alias pst="wl-paste"
alias unlock_transcrypt="transcrypt -y -p "
alias envrc="cp ~/dotfiles/direnv/dotenvrc .envrc"
alias xcurl="curl -x 127.0.0.1:8000 -k "
alias zshconfig="cd ~/zsh && nv"
alias zbench="time zsh -i -c exit"
alias xel="fc"
alias fix_history="rm ~/.zsh_history && cp ~/dotfiles/zsh/.zsh_history ~"
alias sync_history="cp ~/.zsh_history ~/dotfiles/zsh/dot-zsh_history"
alias speaker_connect="bluetoothctl 'connect 10:94:97:36:C7:15'"
alias bezier_python_bininstall="uv pip uninstall bezier && BEZIER_NO_EXTENSION=true uv pip install --upgrade bezier --no-binary=bezier && uv pip install numpy==1.26.4"
